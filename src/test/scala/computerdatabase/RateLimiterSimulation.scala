...
class RateLimiterSimulation extends Simulation {
  ...
  val scn: ScenarioBuilder = scenario("RecordedSimulationWithRateLimiter-1")
    .exec(http("client-1-request")
      .get(endpoint)
      .header("x-client-id","client-1")
      .check(status.is(200)))

  val scn2: ScenarioBuilder = scenario("RecordedSimulationWithRateLimiter-2")
    .exec(http("client-2-request")
      .get(endpoint)
      .header("x-client-id","client-2")
      .check(status.is(200)))

  val scn3: ScenarioBuilder = scenario("RecordedSimulationWithRateLimiter-3")
